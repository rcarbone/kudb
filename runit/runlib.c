...
int run_get_static_sys_info(RunSysStatic *rss)
{
	...
	while (fscanf(fp, "%s", buffer) > 0) {
		if (strstr(buffer, "MemTotal") == buffer) {
			fscanf(fp, "%zd", &(rss->mem_total));
			rss->mem_total *= 1024;
			flag |= 0x1;
		} else if (strstr(buffer, "SwapTotal") == buffer) {
			fscanf(fp, "%zu", &(rss->swap_total));
			rss->swap_total *= 1024;
			flag |= 0x2;
		}
	}
...
int run_get_dynamic_proc_info(pid_t pid, RunProcDyn *rpd)
{
	...
	if (error != KERN_SUCCESS) {
		/* fprintf(stdout, "++ Probably you have to set suid or become root.\n"); */
		rpd->rss = rpd->vsize = 0;
